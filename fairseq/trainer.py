        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    #print(sample['net_input']['src_lengths'].size())
...
class Trainer_Inf(object):
    ...
    def __init__(self, args, task, model, model_E, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task

        self.energy_file = args.energy_file
        # copy model and criterion to current device
        self.criterion = criterion
        self._model = model
        ...
        if args.fp16:
            self._model = self._model.half()
            self._model_E = self._model_E.half()
        if self.cuda:
            self.criterion = self.criterion.cuda()
            self._model = self._model.cuda()
            self._model_E = self._model_E.cuda()
    ...
    def init_meters(self, args):
        self.meters = OrderedDict()
        self.meters['train_loss'] = AverageMeter()
        self.meters['train_discrete_loss'] = AverageMeter()
        self.meters['train_nll_loss'] = AverageMeter()
        self.meters['valid_loss'] = AverageMeter()
        self.meters['valid_discrete_loss'] = AverageMeter()
        self.meters['valid_nll_loss'] = AverageMeter()
        self.meters['wps'] = TimeMeter()       # words per second
        self.meters['ups'] = TimeMeter()       # updates per second
        self.meters['wpb'] = AverageMeter()    # words per batch
    ...
    @property
    def model_E(self):
        if self._wrapped_model_E is None:
            if self.args.distributed_world_size > 1 and not self.args.use_bmuf:
                self._wrapped_model_E = models.DistributedFairseqModel(
                    self.args, self._model_E,
                )
            else:
                self._wrapped_model_E = self._model_E
        return self._wrapped_model_E
    ...
    def load_checkpoint(
        ...
        """Load the energy function for NMT"""
        if os.path.exists(self.energy_file):
            state = checkpoint_utils.load_checkpoint_to_cpu(self.energy_file)
            # load model parameters
            try:
                self.get_model_E().load_state_dict(state['model'], strict=True)
                print("loading the pretrained energy function")
            except Exception:
                raise Exception(
                    'Cannot load energy model parameters from checkpoint, '
                    'please ensure that the architectures match.'
                )
            ...
            print("Cannot find the energy model parameters")
            sys.exit(1)
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.model_E, self.criterion, self.optimizer,
                        ignore_grad
                    )
        ...
        try:
            ...
            self.meters['train_loss'].update(logging_output.get('loss', 0), sample_size)
            self.meters['train_discrete_loss'].update(logging_output.get('discrete_loss', 0), nsentences)
            if 'train_acc' in self.meters:
                self.meters['train_acc'].update(
                    logging_output.get('acc', 0), sample_size)
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.model_E, self.criterion
                )
            except RuntimeError as e:
                if 'out of memory' in str(e) and not raise_oom:
                    print('| WARNING: ran out of memory, retrying batch')
        ...
        ntokens = logging_output.get('ntokens', 0)
        self.meters['valid_loss'].update(logging_output.get('loss', 0), sample_size)
        self.meters['valid_discrete_loss'].update(logging_output.get('discrete_loss', 0), sample_size)
        if 'valid_acc' in self.meters:
            self.meters['valid_acc'].update(
                logging_output.get('acc', 0), sample_size)
    ...
    def get_model_E(self):
        """Get the (non-wrapped) model instance."""
        return self._model_E
