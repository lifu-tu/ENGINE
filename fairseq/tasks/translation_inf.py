...
from fairseq.data import (
    ...
    IndexedRawTextDataset, IndexedCachedDataset, IndexedDataset, LanguagePairSelfDatasetMask, LanguagePairSelfInfDatasetMask
)

from . import FairseqTask, register_task


@register_task('translation_inf')
class TranslationInfTask(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--mask-range', action='store_true',
                            help='dynamic length')
        parser.add_argument('--always-mask', action='store_true',
                            help='mask all the CMLM input')
    ...
    def __init__(self, args, src_dict, tgt_dict):
        ...
        self.always_mask = args.always_mask
    ...
    def load_dataset(self, split, combine=False, epoch=0):
        ...
        #### I modify the following code lines in order to mask all input for the decoder
        ...
        if split=="train":
            self.datasets[split] = LanguagePairSelfInfDatasetMask(
    ...
    def train_step(self, sample, model, model_E, criterion, optimizer, ignore_grad=False):
        """
        ...
        loss, sample_size, logging_output = criterion(model, model_E, sample)
        if ignore_grad:
            loss *= 0
    ...
    def valid_step(self, sample, model, model_E, criterion):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, model_E, sample, train=False)
